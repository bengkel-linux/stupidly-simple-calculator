...
import math
# creating class, Window, and inheriting from the Frame
# class. Frame is a class from the tkinter module.
class Window(Frame):

    def __init__(self, master=None):
        # Parameters for sending through the Frame class.
    ...
    def init_window(self):
        self.focus_set()
        # Set Window Size
        self.master.geometry("369x500")
        # Force Window Size / Disable Maximise
        self.master.resizable(0,0)
        # Set window title
        self.master.title("stupidly-simple-calculator")
        ...
        # SETUP WIDGETS
        message = lambda : messagebox.showinfo(title="About", message="stupidly-simple-calculator gui v.1.0")
        message2 = lambda : messagebox.showwarning(title="Help", message="no")
        message3 = lambda : messagebox.showerror(title="Report Bugs",message="Go To:https://github.com/PaprikaX33/stupidly-simple-calculator")
        menubar = Menu(self)
        menubar.add_command(label="About", command=message) # About menu
        menubar.add_command(label="Help", command=message2)
        menubar.add_command(label="Report Bugs",command=message3)

        # display the menu
        root.config(menu=menubar)
        
        # Configure Display widget
        display = Entry(self, font=("Comic Sans",36,"normal"), width=12)
        display.configure(bg="white",fg="darkblue", relief=FLAT, state="readonly")
        ...
        button_12 = Button(self, text="CE", width=4, height=1,
                          command=self.button_12_handler, bg="darkorange",
                          font=("Comic Sans",20,"bold"))
        ...
        button_EQUALS = Button(self, text="=", width=18, height=1,
                          command=self.button_EQUALS_handler, bg="darkorange",
                          font=("Comic Sans",20,"bold"))
        button_EQUALS.configure(fg="white", relief=FLAT)
        self.button_EQUALS = button_EQUALS

        '''
         MRPLUS, MPREAD, MSUB and MCLEAR isn't fully functional for now it's commented
        ...
        '''
        # Configure button_FACTORIAL widget
        button_FACTORIAL = Button(self, text="Fact",width=18,height=1,
                          command=self.button_FACTORIAL_handler, bg="darkgrey",
                          font=("Comic Sans",20,"bold"))
        button_FACTORIAL.configure(fg="white",relief=FLAT)
        self.button_FACTORIAL = button_FACTORIAL
        ...
        button_3.place(x=180, y=173) #9
        button_ADD.place(x=280, y=287)#+
        button_4.place(x=0, y=230)#4
        button_5.place(x=79, y=230)#5
        button_6.place(x=180, y=230)#6
        button_SUB.place(x=280, y=230)#-
        button_7.place(x=0, y=287)#1
        button_8.place(x=79, y=287)#2
        button_9.place(x=180, y=287)#3
        button_MUL.place(x=280, y=173)#x
        button_10.place(x=79, y=344)#0
        button_11.place(x=0, y=344)#.
        button_12.place(x=0, y=116)#c
        button_DIV.place(x=280, y=116)#/
        button_EQUALS.place(x=0, y=401)#=
        button_PERC.place(x=280, y=344)#%
        button_SQUARE.place(x=79, y=116)
        button_ROOT.place(x=180, y=116)
        button_pi.place(x=180, y=344)#pi
        #button_MPLUS.place(x=180 , y=59)
        #button_MREAD.place(x=79 , y=59)
        #button_MSUB.place(x=280 , y=59)
        #button_MCLEAR.place(x=0 , y=59)
        button_FACTORIAL.place(x=0, y=450)
...
# EVENT HANDLERS
    ...
    def button_FACTORIAL_handler(self):
        self.display.configure(state="normal")
        self.operation = "fact"
    ...
    def button_EQUALS_handler(self):
        ...
        elif self.operation == "fact":
            if num1 < 0:
                print("Sorry,factorial does not exist for negative numbers")
            elif num1 == 0:
                result = 1
            else:
                result = math.factorial(num1-1) * num1
...
def quitprog(event):
    ...
    press = event.char
    if press == '\x1b':
        print('implement quit with esc soon..')
'''
